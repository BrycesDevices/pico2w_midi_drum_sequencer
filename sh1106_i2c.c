...
//--------------------------------------------------------------------+
// _ReidOnlyMemory Functions
//--------------------------------------------------------------------+

void SH1106_drawEmptyStep(sh1106_t *sh1106, uint8_t x, uint8_t y, uint8_t width, uint8_t height) {
    for (int horizontal_index = 0; horizontal_index < width - 2; ++horizontal_index) {
        SH1106_drawPixel(sh1106, x + 1 + horizontal_index, y + 1, STANDARD_COLOR);
        SH1106_drawPixel(sh1106, x + 1 + horizontal_index, y + height - 1, STANDARD_COLOR);
    }
    for (int vertical_index = 0; vertical_index < height - 3; ++vertical_index) {
        SH1106_drawPixel(sh1106, x + 1, y + 2 + vertical_index, STANDARD_COLOR);
        SH1106_drawPixel(sh1106, x + width - 2, y + 2 + vertical_index, STANDARD_COLOR);
    }
};

void SH1106_drawActiveStep(sh1106_t *sh1106, uint8_t x, uint8_t y, uint8_t width, uint8_t height) {
    for(uint8_t i = 0; i < width - 2; ++i)
        for(uint8_t j = 0; j < height - 2; ++j)
            SH1106_drawPixel(sh1106, x + 1 + i, y + 1 + j, STANDARD_COLOR);
};